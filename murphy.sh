...
num=$2
if [ "$3" = "to-murphy" ]; then
	dune exec -- main/main.exe infer consistent data/$name.ml data/$name_assertion$num.ml $name$num.elrond >.out
...
if [ "$3" = "weakening-murphy" ]; then
	...
	dune exec -- main/main.exe infer weakening-murphy data/$name.ml data/$name_assertion$num.ml $host_dir/_$name$num.elrond.spectable $host_dir/_$name$num.elrond.alpha.murphy $name$num.murphy > $name$num.murphy.out
fi
