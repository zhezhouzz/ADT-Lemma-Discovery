...
let pos_verify_update_env ctx vc_env env fv stat =
  ...
  let if_pos = List.fold_left (fun if_pos flow ->
      if if_pos then
        pos_verify_flow ctx vc_env env flow fv stat
      else
        false
        ...
        | PosFv fv ->
          let if_pos =
            clock "pos_verify_update_env"
              (fun _ -> pos_verify_update_env ctx vc_env env fv stat) in
          if not if_pos then find_pos env else NewWeaker env
      in
      match find_pos env with
      | NoWeaker ->
        if !max_loop_counter == 0 then AlreadyMaxed env else NewMaxed (vc_env, env)
      | MayNoWeaker ->
        if !max_loop_counter == 0 then MayAlreadyMaxed env else Weaker (vc_env, env)
      | NewWeaker env ->
